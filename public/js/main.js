document.addEventListener("DOMContentLoaded", () => {
  ...
  function randomNumberGenerator() {
    ...
    while (attempts < maxAttempts) {
      position = Math.floor(Math.random() * gridWidth * gridHeight);
      if (!takenCellsSet.has(position)) {
  ...
  function updateGridDisplay() {
    cells.forEach((cell, index) => {
      //// Foreach cell in cells array, pass player position
      if (index === playerPosition) {
        // If the playersPosition is found in cell array
  ...
  function handleKeyPress(e) {
    ...
    switch (e.key) {
      case "ArrowUp":
        ...
        break;
      case "ArrowDown":
        if (playerPosition < gridWidth * (gridHeight - 1))
          newPlayerPosition += gridWidth;
        break;
      case "ArrowLeft":
    ...
    if (
      ...
      !cells[newPlayerPosition].classList.contains("cell-wall")
    ) {
      //// Check if the new player is not equal to the previous player position, Check if cell contains a wall.
  ...
  function movePlayer(newPlayerPosition) {
    if (cells[newPlayerPosition].classList.contains("cell-enemy")) {
      //// Check if player position collides with the following classes
      ...
      // Otherwise: Update cells to move player
      goToNextLevel();
    } else {
  ...
  function goToNextLevel() {
    takenCellsSet.clear();
    takenCellsSet.add(playerPosition);
    currentLevel++;
